...
def validation_prix_article(prix_article):
    try:
        ...
        print("Erreur !le prix de l'article doit être un nombre.")
        return False
...
def ajouter_produit():
    print("=== === === VEUILLEZ SUIVRE LES INSTRUCTIONS CI_DESSOUS POUR AJOUTER VOTRE PRODUIT=== === ===")
    id_produit = len(produits) + 1
    while True:
        nom = input("veuillez saisir  le nom du produit: ")
        if validation_nom_article(nom):
            break

    # Vérification du produit si il existe
    for produit in produits:
        if produit['nom'].lower() == nom.lower():
            print("desolé! Le produit existe déjà.")
            return

    while True:
        prix = input("veuillez saisir le prix du produit: ")
...
def afficher_produits():
    if not produits:
        print("*** *** ***DESOLE IL Y A PAS D'ARTICLE. *** ***")
    else:
        print("=== === === VOICI LES PRODUITS EXISTANT=== === ===")
        for produit in produits:
            print(f"id_produit : {produit['id_produit']}, Nom: {produit['nom']}, Prix: {produit['prix']}, Quantité: {produit['quantite']}")

def rechercher_produit():
    print("==== === === VEUILLEZ SAISIR LES INFOS SI DESSOUS POUR RECHERCHER UN PRODUIT === === ====")
    choix = input("Voulez-vous rechercher par nom ou par ID? (nom/id): ").lower()
    produit_trouve = None

    if choix == "nom":
        nom = input("veuillez saisir le nom du produit à rechercher: ")
        for produit in produits:
            if produit['nom'].lower() == nom.lower():
    ...
    if produit_trouve:
        ...
        print("==== ==== ==== L'ARTICLE RECHERCHÉ EST INTROUVABLE. ==== ==== ====")

def modifier_produit(id_produit):
    for produit in produits:
        if produit['id_produit'] == id_produit:
            while True:
                nom = input(f"saisisez le nouveau nom de l'article (actuel: {produit['nom']}): ")
                if validation_nom_article(nom):
                    produit['nom'] = nom
                    break
